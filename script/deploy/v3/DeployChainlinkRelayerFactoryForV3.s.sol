...
contract DeployChainlinkRelayerFactoryForV3 is TrebScript, ProxyHelper {
    ...
    function run() public broadcast {
        ...
        bytes memory chainlinkRelayerFactoryInitData = abi.encodeWithSelector(
            IChainlinkRelayerFactory.initialize.selector,
            sortedOracles,
            deployer.account
        );
        chainlinkRelayerFactory = deployer
            .create3(OZTUP_ARTIFACT)
            .setLabel("ChainlinkRelayerFactory")
            .deploy(
                abi.encode(
                    chainlinkRelayerFactoryImpl,
                    proxyAdmin,
                    chainlinkRelayerFactoryInitData
                )
            );
    }
}
