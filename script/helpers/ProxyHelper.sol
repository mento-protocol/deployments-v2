...
string constant OZTUP_ARTIFACT =
...
interface ILegacyProxy {
    ...
    function _setAndInitializeImplementation(address implementation, bytes calldata init) external;
...
contract ProxyHelper is TrebScript, ProxyViewHelper {
    ...
    constructor() {
        string memory proxyTypeFromEnv = vm.envOr("defaultProxyType", string(""));
        if (bytes(proxyTypeFromEnv).length > 0) {
            if (keccak256(bytes(vm.envString("defaultProxyType"))) == keccak256("CELO")) {
                defaultProxyType = ProxyType.CELO;
            } else if (keccak256(bytes(vm.envString("defaultProxyType"))) == keccak256("OZTUP")) {
    ...
    function lookupProxy(string memory contractName) internal view returns (address proxy) {
        address celo = lookup(string.concat(CELO_LOOKUP_PREFIX, contractName));
        address oztup = lookup(string.concat(OZTUP_LOOKUP_PREFIX, contractName));
        require(
    ...
    function lookupProxy(string memory contractName, string memory namespace) internal view returns (address proxy) {
        proxy = lookupProxy(contractName, defaultProxyType, namespace);
    }

    function lookupProxy(string memory contractName, ProxyType _proxyType, string memory namespace)
        internal
        view
    ...
    function lookupProxy(string memory contractName, ProxyType _proxyType) internal view returns (address proxy) {
        string memory identifier;
        if (_proxyType == ProxyType.CELO) {
            identifier = string.concat(CELO_LOOKUP_PREFIX, contractName);
        } else if (_proxyType == ProxyType.OZTUP) {
    ...
    function lookupProxyOrFail(string memory contractName, string memory namespace)
        internal
        view
        returns (address proxy)
    {
    ...
    function lookupProxyOrFail(string memory contractName, ProxyType _proxyType, string memory namespace)
        internal
        view
        returns (address proxy)
    {
    ...
    function lookupProxyOrFail(string memory contractName, ProxyType _proxyType) internal view returns (address proxy) {
        string memory identifier;
        if (_proxyType == ProxyType.CELO) {
            identifier = string.concat(CELO_LOOKUP_PREFIX, contractName);
        } else if (_proxyType == ProxyType.OZTUP) {
    ...
    function lookupOrFail(string memory identifier, string memory namespace) internal view returns (address addy) {
    ...
    function predictProxy(Senders.Sender storage deployer, string memory label) internal returns (address proxy) {
        return predictProxy(defaultProxyType, deployer, label);
    }

    function predictProxy(ProxyType _proxyType, Senders.Sender storage deployer, string memory label)
        internal
        returns (address proxy)
    {
        if (_proxyType == ProxyType.CELO) {
    ...
    function deployProxy(
        ...
        return deployProxy(defaultProxyType, deployer, label, implementation, initData);
    }
    ...
    function deployCeloProxy(
        ...
        proxy = deployer.create3(CELO_ARTIFACT).setLabel(label).deploy(abi.encode(deployer.account));
    ...
    function verifyProxyAdmin(string memory identifier, address proxy) internal view {
        address proxyAdmin = lookup("ProxyAdmin");
        address currentProxyAdmin = getProxyAdmin(proxy);
        require(
            currentProxyAdmin == proxyAdmin,
            string.concat(
                identifier, " proxy admin mismatch\n", currentProxyAdmin.toHexString(), " != ", proxyAdmin.toHexString()
            )
        );
    }

    function verifyProxyImpl(string memory identifier, address proxy, address expectedImpl) internal view {
        address actualImpl = getProxyImplementation(proxy);
        require(actualImpl == expectedImpl, string.concat(identifier, " proxy implementation mismatch"));
    }
}
