...
library Anvil {
    ...
    function setErc20Balance(address token, address wallet, uint256 value, bytes32 slot) internal {
        vm.store(token, keccak256(abi.encode(wallet, slot)), bytes32(value));
    }

    function setErc20BalanceRpc(address token, address wallet, uint256 value, bytes32 slot) internal {
    ...
    function getStorageAt(address target, bytes32 slot) internal view returns (bytes32) {
        ...
        (bool success,) = address(vm)
            .call(
                abi.encodeWithSignature(
                    "rpc(string,string)",
                    "anvil_setCode",
                    string(abi.encodePacked('["', vm.toString(target), '","', vm.toString(code), '"]'))
                )
            );
