...
import {ITradingLimitsV2} from "mento-core/interfaces/ITradingLimitsV2.sol"; // ADDED
...
/**
 * @title FinishEuropFPMM
 * @notice One-shot recovery script: finishes configuring the EURm/EUROP FPMM on
 *         polygon_testnet. Delete once it has been run.
 *
 * @dev A verbatim copy of CreateFPMM with the smallest possible diff. The CreateFPMM run
 *      that deployed this pool reverted partway through `_configureTradingLimits` (the
 *      token1 call ran out of gas), leaving the pool created and seeded but unconfigured.
 *      CreateFPMM cannot resume it: it skips any pool that already exists, so the
 *      remaining steps would never run.
 *
 *      Diff vs CreateFPMM (all marked CHANGED/ADDED inline):
 *        1. run() selects ONLY the EURm/EUROP config; no other pool is read or touched.
 *        2. _deployFPMM() requires the pool to already exist, and skips creation + the
 *           initial-liquidity mint (both one-shot, already landed on-chain).
 *        3. Steps that already landed are skipped rather than re-sent -- addPool in
 *           particular reverts LS_POOL_ALREADY_EXISTS if called twice.
 *      Everything else, including _verifySwap and the rebalance verifications, is
 *      byte-for-byte the original.
 */
contract FinishEuropFPMM is TrebScript, ProxyHelper, ConfigHelper, StdCheats {
    using Deployer for Senders.Sender;
    using Senders for Senders.Sender;

    address constant SWAP_TEST_ACCOUNT = address(1337);

    // ADDED: identifies the single pool this script is allowed to touch.
    ...
    function run() public virtual broadcast {
        ...
        // CHANGED: CreateFPMM loops over every FPMMConfig. We run the EURm/EUROP one only.
        _deployFPMM(_europConfig());
    }

    /// @dev ADDED: pick the EURm/EUROP config out of the FPMM configs, and only that one.
    function _europConfig() internal returns (IMentoConfig.FPMMConfig memory) {
        address collateral = config.getAddress(COLLATERAL_SYMBOL);
        address debt = config.getAddress(DEBT_SYMBOL);

        IMentoConfig.FPMMConfig[] memory fpmmConfigs = config.getFPMMConfigs();
        for (uint256 i = 0; i < fpmmConfigs.length; i++) {
            bool matches = (fpmmConfigs[i].token0 == collateral && fpmmConfigs[i].token1 == debt)
                || (fpmmConfigs[i].token0 == debt && fpmmConfigs[i].token1 == collateral);
            if (matches) {
                return fpmmConfigs[i];
            }
        }
        revert("No EURm/EUROP FPMMConfig found");
    }

    function _deployFPMM(IMentoConfig.FPMMConfig memory cfg) internal {
        address fpmmProxy = factory.getPool(cfg.token0, cfg.token1);

        // CHANGED: CreateFPMM bails out here when the pool exists. We do the opposite --
        // the pool MUST already exist, and we skip _createFPMM (deploy + initial liquidity
        // mint), which is one-shot and already landed. The rest of the flow is unchanged.
        require(fpmmProxy != address(0), "EUROP FPMM does not exist -- run CreateFPMM instead");
        console.log(
            "  > resuming configuration for existing fpmm (%s, %s)",
    ...
    function _configureTradingLimits(address fpmmProxy, IMentoConfig.FPMMConfig memory cfg) internal {
        ...
        // CHANGED: skip only when the on-chain limit already matches the config. If it's unset
        // or has drifted from the config, reconfigure it.
        if (hasToken0Limits && _tradingLimitMatches(fpmmProxy, cfg.token0, limits.token0Limit0, limits.token0Limit1)) {
            console.log("  > Trading limits already match config for token0 (%s)", IERC20Metadata(cfg.token0).symbol());
        } else if (hasToken0Limits) {
        ...
        if (hasToken1Limits && _tradingLimitMatches(fpmmProxy, cfg.token1, limits.token1Limit0, limits.token1Limit1)) {
            console.log("  > Trading limits already match config for token1 (%s)", IERC20Metadata(cfg.token1).symbol());
        } else if (hasToken1Limits) {
    ...
    /// @dev ADDED: true only if the on-chain trading limit for `token` already matches what the
    /// config asks for. configureTradingLimit stamps the token's decimals into the stored config
    /// and scales the limits to TradingLimitsV2's internal 15-decimal precision, so we apply the
    /// same transform to the config values before comparing. A zero `decimals` means it was never
    /// configured, and can never match since validate() rejects decimals == 0.
    function _tradingLimitMatches(address fpmmProxy, address token, uint256 limit0, uint256 limit1)
        internal
        view
        returns (bool)
    {
        (ITradingLimitsV2.Config memory onChain,) = IFPMM(fpmmProxy).getTradingLimits(token);
        if (onChain.decimals == 0) {
            return false;
        }

        uint8 decimals = IERC20Metadata(token).decimals();
        uint256 scaled0 = (limit0 * 1e15) / 10 ** decimals;
        uint256 scaled1 = (limit1 * 1e15) / 10 ** decimals;

        return onChain.decimals == decimals && uint256(uint120(onChain.limit0)) == scaled0
            && uint256(uint120(onChain.limit1)) == scaled1;
    }
    ...
    function _setupReserveLiquidityStrategy(
        ...
        // CHANGED: guarded so a partial re-run doesn't re-send it.
        if (IFPMM(fpmmProxy).liquidityStrategy(strategyAddy)) {
            console.log("  > Liquidity strategy already set on FPMM:", strategyAddy);
        } else {
        ...
        // CHANGED: guarded -- addPool reverts LS_POOL_ALREADY_EXISTS if called twice.
        if (ILiquidityStrategy(strategyAddy).isPoolRegistered(fpmmProxy)) {
            console.log("  > Pool already registered on ReserveLiquidityStrategy");
        } else {
