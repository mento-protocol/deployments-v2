...
contract RevertSnapshot is Script {
    function run() public {
        ...
        try vm.readFile("cache/.tenderly-snapshot") returns (string memory fileContent) {
            snapshotId = fileContent;
            console.log("Using snapshot ID from .tenderly-snapshot:", snapshotId);
        } catch {
            // If file doesn't exist, check if snapshot ID is provided as env variable
            snapshotId = vm.envString("SNAPSHOT_ID");
            console.log("Using snapshot ID from SNAPSHOT_ID env var:", snapshotId);
        }
