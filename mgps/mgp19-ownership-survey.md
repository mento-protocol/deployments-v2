...
- Ordering: MGP-18 and MGP-19 are staggered far enough apart that no on-chain ordering enforcement is needed; MGP-19 is only proposed after MGP-18 has executed. (`src/ProposalDependencyGuard.sol` remains in source, with unit and fork tests, for future proposals that do need to be voted concurrently with an execution-order constraint.)
